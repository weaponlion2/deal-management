...
import { Grid2 as Grid, TextField, Button, Typography, Box, Paper, Divider, Link, Tabs, Tab, Card, CardContent } from '@mui/material';
import { commentForm, fetchComment, fetchOptions, Sources, Statuses, TaskForm } from './Ticket';
...
export const SourcesNode: Record<typeof Sources[number]['id'] | "-1", ReactNode> =
{
...
const TicketView: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    const data: commentForm = {
      comments: comment, cid: 0, id: Number(uid), uid: Number(localStorage.getItem("@Id") ?? 0), type: "TICKET", createOn: dayjs().format("YYYY-MM-DD"), isactive: true, userName: "You"
    }
      ...
      if (req.status === 200) {
        ...
        if (status === "Success") {
          if (typeof data !== "undefined") {
            ...
            console.log("selced",selectedPipeline)
            setTicketData({...data[0] ,pipeline: selectedPipeline ? `${selectedPipeline.name}` : "-"})
            if (data[0]['taskmodel']) {
  ...
  return (
    <Paper elevation={3} sx={{ borderRadius: 2, pb: 0 }}>
      ...
      <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
        <Tab label="Details" id={`simple-tab-1`} aria-controls={`simple-tabpanel-1`} />
        <Tab label="Tasks" id={`simple-tab-3`} aria-controls={`simple-tabpanel-2`} />
        <Tab label="Comment" id={`simple-tab-3`} aria-controls={`simple-tabpanel-3`} />
      </Tabs>
      ...
      >
        {value === 0 &&
          <>
            <Box display="flex" flexDirection="column" gap={2} paddingX={2} sx={{ borderTop: "1px solid rgba(224, 224, 224, 1)" }}>
              <Grid container>
                ...
                <Grid size={{ xs: 12, sm: 6 }} sx={{ p: 1 }}>
                  <Grid container>
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Typography variant='caption' color='textSecondary' fontSize={17}>{ticketData.contactFirstName === "" ? NA : ticketData.contactFirstName} {ticketData.contactLastName} </Typography>
                    </Grid>
                  </Grid>
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }} sx={{ borderRight: "1px solid lightgrey", p: 1 }}>
                  <Grid container>
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Typography variant='caption' color='textSecondary' fontSize={17}>{SourcesNode[ticketData.source as typeof Sources[number]['id'] | "-1"]}</Typography>
                    </Grid>
                  </Grid>
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }} sx={{ p: 1 }}>
                  <Grid container>
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Typography variant='caption' color='textSecondary' fontSize={17}>{ticketData.priority}</Typography>
                    </Grid>
                  </Grid>
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }} sx={{ borderRight: "1px solid lightgrey", p: 1 }}>
                  <Grid container>
                    ...
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Typography variant='caption' color='textSecondary' fontSize={17}>{ticketData.pipeline}</Typography>
                    </Grid>
                  </Grid>
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }} sx={{ p: 1 }}>
                ...
                <Grid size={{ xs: 12, sm: 6 }} sx={{ p: 1 }}>
                  <Grid container>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Typography variant='subtitle2' fontSize={17}>Open / Close Date</Typography>
                    </Grid>
                    <Grid size={{ xs: 12, sm: 6 }}>
                      <Typography variant='caption' color='textSecondary' fontSize={17}>{ticketData.openDate === "" ? NA : ticketData.openDate} / {(ticketData.closeDate === "" || !ticketData.closeDate) ? NA : ticketData.closeDate}</Typography>
                    </Grid>
                  </Grid>
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }} sx={{ borderRight: "1px solid lightgrey", p: 1 }}>
      ...
      >
        {value === 1 && <>
          <Box gap={2} paddingX={1} py={2} sx={{ borderTop: "1px solid rgba(224, 224, 224, 1)", display: "flex", gap: 2 }}>
            {taskData.map((v) =>
              <Card sx={{ minWidth: 345 }} key={v.id}>
                <CardContent>
                  <Typography gutterBottom sx={{ color: 'text.secondary', fontSize: 14 }}>
                    ⌘ {v.name}
                  </Typography>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography variant="subtitle1" color='textSecondary' fontWeight={700}>{"Created On"} : </Typography>
                    <Typography variant="body1">{ticketData.createdOn}</Typography>
                  </Box>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography variant="subtitle1" color='textSecondary' fontWeight={700}>{"Owner Name"} : </Typography>
                    <Typography variant="body1">{ticketData.ownerName}</Typography>
                  </Box>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography variant="subtitle1" color='textSecondary' fontWeight={700}>{"Status"} : </Typography>
                    <Typography variant="body1">{StatusTypography[ticketData.status]}</Typography>
                  </Box>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <Typography variant="subtitle1" color='textSecondary' fontWeight={700}>{"Product"} : </Typography>
                    <Typography variant="body1">{ticketData.productName}</Typography>
                  </Box>
                </CardContent>
              </Card>
            )}
            {taskData.length === 0 && <Typography>No task found</Typography>}
          </Box>
        </>}
      </div>
      ...
      >
        {value === 2 && <>
          <Box display="flex" flexDirection="column" gap={2} paddingY={1} sx={{ borderTop: "1px solid rgba(224, 224, 224, 1)" }}>
            ...
            <Box display="flex" alignItems="center" paddingTop={2}>
              <TextField
                ...
                sx={{ width: "85%", mx: 3 }}
              />
              <Button disabled={comment ? false : true} variant='contained' color="primary" onClick={() => handleSubmit()}>
                Submit
              </Button>
            </Box>
            {/* Comment Box */}
            <Box display="flex" flexDirection="column" gap={1} justifyContent={"center"} width={"100%"}>
              <Grid size={4}>

                <Box sx={{ maxHeight: 450 }}>
                  <Divider />
                  <Box sx={{ overflow: "auto", maxHeight: 430, px: 1 }}>
                    <Timeline sx={{ p: 0 }}>
                      {
                        commentList.map((v) => (
                    ...
                    {
                      (commentList.length === 0) && <Typography color='warning' variant='h5' >No comments found</Typography>
                    }
                  </Box>
                </Box>
              </Grid>
